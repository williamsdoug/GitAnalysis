#
# Git_Extract_Join.py - Code to extract Git commit data, metadata, diff and blame.
#
...
# - 8/31/14: only compute blame for selected file types (source code files) to avoid git
...
def parse_msg(msg, patch=False):
    """Overall routine for processing commit messages, also used for patch messages"""
    result = {}
    if not msg:
        return {}
    for line in msg.split('\n'):
        ...
        if patch:
            if line.startswith('From: '):
                try:
                    #'<git.Actor "'+ obj.name.encode('ascii', 'ignore') +' <'+ obj.email+'>">'
                    result.update({'pAuth':'<git.Actor "'+line[len('From: '):]+'">'})
                except Exception:
                    pass
...
def parse_diff(d, proximity_limit=4):
    """Parses individual git diff string, returning range and line proximity to changes"""
    p_pos = 0
    n_pos = 0
    changes = []
    line_range = []
...
def assign_blame(d, p_cid, repo_name=''):
    """Combine diff with blame for a file"""
    try:
        result = parse_diff(d)
        #now annotate with blame information
        blame = dict([(int(x['lineno']), x) for x in get_blame(p_cid, d.b_blob.path, repo_name=repo_name)])
...
def process_commit_details(cid, repo_name='', filt=['py', 'sh', 'js', 'c', 'go', 'sh', 'conf']):
    ...
    for p in c.parents:    #iterate through each parent
        ...
        for d in i.iter_change_type('M'): 
            if d.b_blob and d.b_blob.path.split('.')[-1] in filt:       
                blame.append(assign_blame(d,p.hexsha,repo_name=repo_name))
        
        
        #blame += [assign_blame(d,p.hexsha,repo_name=repo_name) for d in i.iter_change_type('M') if d.b_blob]   
...
def compute_all_blame(bug_fix_commits, start = 0, limit = 1000000, repo_name='',
    ...
    for cid in bug_fix_commits[start:start+limit]:
        all_blame.append({'cid':cid, 'blame': process_commit_details(cid,repo_name=repo_name)})
...
def build_all_blame(project, combined_commits, update=True, repo_name=''):
    ...
    bug_fix_commits = set([k for k,v in combined_commits.items() if 'lp:id' in v])
    print 'print bug fix commits:', len(bug_fix_commits)
...
#
# Routines for extracting additional metadata from Jenkins for Jenkins authored commits
#
...
def update_commits_with_patch_data(commits, project='nova'):
    ...
    patch_commits = dict([(x['cid'], parse_msg(x['patch'], patch=True)) 
                     for x in load_patch_data(jenkins_commits, project=project, incremental=True)])
...
#
# Join Code - produces combined_commits (joins LP Bugs & Gerrit with Commit data)
#
    
def build_joined_LP_Gerrit_git(project, commits, downloaded_bugs, all_change_details):
